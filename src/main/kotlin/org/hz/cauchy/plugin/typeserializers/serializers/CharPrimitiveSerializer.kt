...
/**
 * Char基本类型序列化器
 * 特殊处理char类型，因为Parcel没有直接的char方法
 */
class CharPrimitiveSerializer : TypeSerializer {

    override fun writeValue(field: SerializableValue, parcel: String, flags: String): String {
        return "$parcel.writeInt(${field.name}.code);"
    }

    override fun readValue(field: SerializableValue, parcel: String): String {
        return "${field.simpleName} = $parcel.readInt().toChar();"
    }
}
